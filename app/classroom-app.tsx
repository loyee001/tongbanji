...
function ClassroomWorkspace({classroom}:{classroom:ReturnType<typeof useClassroom>}){
 ...
 return <SidebarProvider style={{'--sidebar-width':'13.5rem'} as CSSProperties} className="app-shell">
  ...
  <ClassroomDialogs key={dialog||'closed'} data={data} dialog={dialog==='manageRules'?null:dialog} setDialog={setDialog} week={week} user={user} busy={busy} mutate={mutate} onRule={chooseRule} editingRule={draftItems.find(item=>item.key===editingItem)?.rule} onVoid={onVoid}/>{dialog==='manageRules'&&<RulesManager data={data} busy={busy||submitting} setData={setData} mutate={mutate} reload={reload} onClose={()=>setDialog(null)}/>}<Toaster richColors position="bottom-right"/>
 </SidebarProvider>;
}
